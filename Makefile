...
deps.darwin.amd64:
	...
	clang++ -std=c++11 -O0 --target=x86_64-apple-macos13 -DGODUCKDB_FROM_SOURCE -c duckdb.cpp
...
deps.darwin.arm64:
	if [ "$(shell uname -s | tr '[:upper:]' '[:lower:]')" != "darwin" ]; then echo "Error: must run build on darwin"; false; fi
	clang++ -std=c++11 -O0 --target=arm64-apple-macos13 -DGODUCKDB_FROM_SOURCE -c duckdb.cpp
	ar rvs libduckdb.a duckdb.o
...
deps.linux.amd64:
	...
	clang++ -std=c++11 -O0 -DGODUCKDB_FROM_SOURCE -c duckdb.cpp
	ar rvs libduckdb.a duckdb.o
